...
//				cameraEntering3DAnimation = true;
	...
	/// <summary>
	/// Maxs the animation time reached.
	/// </summary>
	/// <returns><c>true</c>, if animation time reached was maxed, <c>false</c> otherwise.</returns>
	bool MaxAnimationTimeReached ()	{
		return (Time.time - timeAtStartOf3DAnimation) >= MAX_ANIMATION_TIME; 
	}

	/// <summary>
	/// Maxs the entering cam animation time reached.
	/// </summary>
	/// <returns><c>true</c>, if entering cam animation time reached was maxed, <c>false</c> otherwise.</returns>
